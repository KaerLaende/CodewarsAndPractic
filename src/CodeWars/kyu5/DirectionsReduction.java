...
public class DirectionsReduction {
    ...
    public static void main(String[] args) {
        String[] test = new String[]{"NORTH", "SOUTH", "SOUTH", "EAST", "WEST", "NORTH"};
        String[] test2 = new String[]{"NORTH", "SOUTH", "SOUTH", "EAST", "WEST", "NORTH", "WEST"};
        System.out.println(Arrays.toString(dirReduc(test2)));
    }

    public static String[] dirReduc(String[] arr) {
        ArrayList<String> newArray = new ArrayList<>(Arrays.asList(arr));
        for (int i = 0; i < newArray.size(); i++) {
            if ((newArray.size()-i) > 1) {
                if (newArray.get(i).equals("NORTH") && newArray.get(i + 1).equals("SOUTH")
                        || newArray.get(i).equals("SOUTH") && newArray.get(i + 1).equals("NORTH")
                        || newArray.get(i).equals("EAST") && newArray.get(i + 1).equals("WEST")
                        || newArray.get(i).equals("WEST") && newArray.get(i + 1).equals("EAST")) {
                    newArray.remove(i + 1);
                    newArray.remove(i);
                    i = -1;
                }
